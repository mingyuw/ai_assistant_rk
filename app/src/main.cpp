...
{
  struct decoder *dec = (decoder *)user_data;
...
video_init ()
{
  ...
  static const char *pipeline = NULL;
	std::string rtsp_src_addr = "rtsp://admin:htzm123456@112.44.218.103:354/0/0/0";//绵阳
	//std::string rtsp_src_addr = "rtsp://admin:htzm123456@171.221.205.72:14154";//华体
	//std::string rtsp_src_addr = "rtsp://192.168.5.17";//xilinx实验室
	//std::string rtsp_src_addr = " rtsp://admin:htzm123456@183.252.19.171:12004";//福州
	std::string file_src_addr = "/home/linaro/Downloads/local2-1.mp4";
	std::string uvc_src_addr = "/dev/video0";
  ...
  char file_str[512] = {0};
	...
	sprintf(file_str, \
        "filesrc location=%s ! rtph265depay ! h265parse ! \
        mppvideodec ! videoconvert ! appsink", \
        file_src_addr.c_str());
	
  sprintf(rtsp_str, 
        "rtspsrc location=%s name=\"src\" ! \
        rtph265depay ! h265parse ! \
        mppvideodec name=\"decode\" ! \
        videoconvert ! \
        appsink sync=false name=\"sink\"", \
        rtsp_src_addr.c_str());
	
  sprintf(uvc_str, 
          "v4l2src device=%s name=\"src\" ! video/x-raw, width=1280, height=720, \
            format=YUY2, framerate=25/1 ! videoconvert ! video/x-raw, \
            width=1280, height=720, format=BGRx ! videoconvert ! appsink", \
            uvc_src_addr.c_str());
...
buffer_to_file (struct decoder *dec, GstBuffer * buf)
{
  ...
  g_snprintf (filename, sizeof (filename), "img%05d.%s", dec->frame, pixfmt_str);
...
{
  struct decoder *dec = (decoder *)arg;
  do {
    GstSample *samp;
    GstBuffer *buf;
